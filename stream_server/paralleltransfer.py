...
if connection_limit > 25:
    root_log.warning(
        "The connection limit should not be set above 25 to avoid"
...
class DCConnectionManager:
    ...
    async def _new_connection(self) -> Connection:
        ...
        conn = Connection(
            sender=sender, log=self.log.getChild(f"conn{index}"), lock=asyncio.Lock()
        )
    ...
    async def _export_auth_key(self, conn: Connection) -> None:
        self.log.info(
            f"Exporting auth to DC {self.dc.id}"
    ...
    async def _next_connection(self) -> Connection:
        ...
        for conn in self.connections:
            if not best_conn or conn.users < best_conn.users:
                best_conn = conn
        if (not best_conn or best_conn.users > 0) and len(
            self.connections
        ) < connection_limit:
...
class ParallelTransferrer:
            ...
            async with dcm.get_connection() as conn:
                ...
                while part <= last_part:
                    ...
                    if part == first_part:
                        ...
                        yield result.bytes
                    log.debug(
                        f"Part {part}/{last_part} (total {part_count}) downloaded"
                    )
                    part += 1
    ...
    def download(
        self, file: TypeLocation, file_size: int, offset: int, limit: int
    ) -> AsyncGenerator[bytes, None]:
