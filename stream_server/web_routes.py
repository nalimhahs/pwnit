...
async def generate_url_from_id(message_id):
    message = await client.get_messages("me", ids=message_id)
    url = public_url / "stream" / str(pack_id(message)) / get_file_name(message)
    return url
...
async def handle_request(req: web.Request, head: bool = False) -> web.Response:
    ...
    if not head:
        ...
        body = transfer.download(
            message.media, file_size=size, offset=offset, limit=limit
        )
    else:
        body = None
    return web.Response(
